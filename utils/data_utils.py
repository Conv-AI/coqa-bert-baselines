...
def prepare_datasets(config, tokenizer_model):
    ...
    trainset = CoQADataset(config['trainset'], config['num_examples'])
...
class CoQADataset(Dataset):
    ...
    def __init__(self, filename, num_examples):
        ...
        for paragraph in tqdm(dataset['data'][:self.num_examples]):
            # print(paragraph)
            history = []
            for qas in paragraph['qas']:
                qas['paragraph_id'] = len(self.paragraphs)
                temp = []
                # if config['n_history'] < 0 else min(config['n_history'], len(history))
                ...
                history.append((qas['annotated_question']
                                ['word'], qas['annotated_answer']['word']))
                qas['annotated_question']['word'] = temp
                self.examples.append(qas)
                question_lens.append(len(qas['annotated_question']['word']))
                paragraph_lens.append(
                    len(paragraph['annotated_context']['word']))
        ...
        print('Load {} paragraphs, {} examples.'.format(
            len(self.paragraphs), len(self.examples)))
        print('Paragraph length: avg = %.1f, max = %d' %
              (np.average(paragraph_lens), np.max(paragraph_lens)))
        print('Question length: avg = %.1f, max = %d' %
              (np.average(question_lens), np.max(question_lens)))
        # timer.finish()
    ...
    def chunk_paragraphs(self, tokenizer, model_name):
        ...
        c_known = 0
        dis = 0
        for i, ex in tqdm(enumerate(self.examples[:self.num_examples])):
            ...
            paragraph = self.paragraphs[ex['paragraph_id']
                                        ]['annotated_context']['word']
            ...
            for spans in doc_spans:
                ...
                _example = {'tokens': tokens, 'answer': tokens[start: end + 1], 'actual_answer': ex['answer'],
                            'input_tokens': input_ids, 'input_mask': input_mask, 'segment_ids': segment_ids, 'start': start, 'end': end}
