...
class ModelHandler():
    def __init__(self, config):
        ...
        self.model = Model(
            config, MODELS[config['model_name']], self.device, tokenizer).to(self.device)
        t_total = len(
            self.train_loader) // config['gradient_accumulation_steps'] * config['max_epochs']
        self.optimizer = AdamW(self.model.parameters(),
                               lr=config['lr'], eps=config['adam_epsilon'])
    ...
    def train(self):
        if not self.restored:
            ...
            self._run_epoch(self.dev_loader, training=False,
                            verbose=self.config['verbose'], save=False)

            format_str = "Validation Epoch {} -- F1: {:0.2f}, EM: {:0.2f} --"
            print(format_str.format(self._epoch,
                                    self._dev_f1.mean(), self._dev_em.mean()))
        ...
        while self._stop_condition(self._epoch):
            ...
            self._run_epoch(self.train_loader, training=True,
    ...
    def restore(self):
        if not os.path.exists(self.config['pretrained_dir']):
            ...
            return
        restored_params = torch.load(
            self.config['pretrained_dir']+'/latest/model.pth')
    ...
    def save(self, save_epoch_val):
        ...
        if self._best_epoch == self._epoch:
            ...
            torch.save(
                save_dic, self.config['save_state_dir']+'/best/model.pth')
        ...
        self.tokenizer.save_pretrained(self.config['save_state_dir'])
    ...
    def _run_epoch(self, data_loader, training=True, verbose=10, out_predictions=False, save=True):
        ...
        while data_loader.batch_state < len(data_loader):
            ...
            f1, em = self.model.evaluate(
                start_logits, end_logits, paragraphs, answers)

            self._update_metrics(tr_loss, f1, em, len(
                paragraphs), training=training)
            ...
            if (verbose > 0) and (data_loader.batch_state % verbose == 0):
                ...
                print(self.report(data_loader.batch_state,
                                  tr_loss, f1 * 100, em * 100, mode))
